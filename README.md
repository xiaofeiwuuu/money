...
- [x] 微信账单 CSV/XLSX 解析
- [ ] 银行卡账单 CSV 解析（招行、工行等）
- [x] 格式版本适配机制（ParserConfig 动态配置）

#### 1.2 数据去重
- [ ] 组合字段去重（时间 + 金额 + 商户 + 备注）
- [x] 交易号去重（source + order_id 唯一索引）
- [ ] 重复数据预览确认
...
### 后端
- [x] **技术选型**：Python FastAPI + SQLAlchemy 2.0 (异步)
- [x] **数据库**：PostgreSQL + asyncpg
- [ ] **云服务**：阿里云 / 腾讯云
- [x] **已实现**：
  - CSV/XLSX 解析与去重
  - 数据存储 (Alembic 迁移)
...
|------|----------|
| 2024-XX-XX | 初始需求整理 |
| 2026-04-24 | 后端 MVP 完成：CSV 解析、去重、认证、CI/CD |
