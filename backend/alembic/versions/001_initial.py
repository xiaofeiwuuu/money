...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "family_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("families.id"), nullable=False
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_family_members_family_user", "family_members", ["family_id", "user_id"], unique=True
    )
    ...
    op.create_index(
        ...
        "transactions",
        ["user_id", "source", "source_order_id"],
        unique=True,
    )
